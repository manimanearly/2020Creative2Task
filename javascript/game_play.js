...
function box_select(num){
    ...
    for(i = 0; i < select_box.length; i++){
        ...
        if(select_box[i] == 0){
            select_box[i] = num;
            select_box_img[i] = document.getElementById("box" + num);
            break;
        }
    }
}
...
function move(){
    count += 0.1;
    let img_pos = [select_box_img[0].getBoundingClientRect(), select_box_img[1].getBoundingClientRect()];
    let set_pos = move_lerp([img_pos[0].left, img_pos[0].top], [img_pos[1].left, img_pos[1].top], count);
    select_box_img[0].style.top = set_pos[1];
    select_box_img[0].style.left = set_pos[0];

    set_pos = move_lerp([img_pos[1].left, img_pos[1].top], [img_pos[0].left, img_pos[0].top], count);
    select_box_img[1].style.top = set_pos[1];
    select_box_img[1].style.left = set_pos[0];
}

function move_lerp(pos_a, pos_b, ratio){
    let calc_vec = [pos_a[0] - pos_b[0], pos_a[1] - pos_b[1]];
    let base_pos = [pos_a[0] - (calc_vec[0] / 2), pos_a[1] - (calc_vec[1] / 2)];
    let angle = Math.atan2(calc_vec[1], calc_vec[0]) + (Math.PI / 180) * ratio;
    let dis = Math.sqrt(calc_vec[0] * calc_vec[0] + calc_vec[1] * calc_vec[1]) / 2;
